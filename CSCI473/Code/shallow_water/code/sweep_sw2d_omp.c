// sweep_sw2d_omp.c — OpenMP variant of sw2d (linearized shallow-water solver)
// Build: cc -O3 -march=native -Wall -Wextra -fopenmp -o sw2d_omp sweep_sw2d_omp.c -lm
...
static void usage(const char *prog) {
    fprintf(stderr,
            ...
            "  --threads INT         OpenMP threads [max]\n"
...
int main(int argc, char **argv) {
    ...
    const double inv2dy = 1.0 / (2.0 * P.dy);
    const double bytes_per_cell_update_est = 3.0 * (5 + 1) * 8.0;
    double ema_updates = 0.0, ema_GBps = 0.0;
...
#pragma omp parallel default(none)                                                               \
    shared(R, C, P, h, u, v, h_new, u_new, v_new, inv2dx, inv2dy, fout, bytes_per_cell_update_est, ema_alpha, \
           nframes, last_t, ema_updates, ema_GBps, stderr)
    {
        for (int step = 1; step <= P.steps; ++step) {
#pragma omp for collapse(2)
            for (int r = 0; r < R; ++r) {
                for (int c = 0; c < C; ++c) {
                    double hL = getH(h, r, c - 1, R, C);
                    double hR = getH(h, r, c + 1, R, C);
                    double hD = getH(h, r - 1, c, R, C);
                    double hU = getH(h, r + 1, c, R, C);
                    double dhdx = (hR - hL) * inv2dx;
                    double dhdy = (hU - hD) * inv2dy;
                    int id = idx(r, c, C);
                    u_new[id] = u[id] + (-P.g * dhdx) * P.dt;
                    v_new[id] = v[id] + (-P.g * dhdy) * P.dt;
                }
            }
#pragma omp for collapse(2)
            for (int r = 0; r < R; ++r) {
                for (int c = 0; c < C; ++c) {
                    double uL = getH(u_new, r, c - 1, R, C);
                    double uR = getH(u_new, r, c + 1, R, C);
                    double vD = getH(v_new, r - 1, c, R, C);
                    double vU = getH(v_new, r + 1, c, R, C);
                    double dudx = (uR - uL) * inv2dx;
                    double dvdy = (vU - vD) * inv2dy;
                    int id = idx(r, c, C);
                    h_new[id] = h[id] + (-P.H0 * (dudx + dvdy)) * P.dt;
                }
            }
#pragma omp single
            {
                ...
                if (fout && P.save_interval > 0 && (step % P.save_interval == 0)) {
                    size_t count = (size_t)R * (size_t)C;
                ...
                if (P.progress_bar || (P.stats_interval > 0 && (step % P.stats_interval) == 0)) {
                    double t1 = now_sec();
                    double dt_wall = t1 - last_t;
                    last_t = t1;
                    if (dt_wall > 0) {
                        double upd = (double)R * (double)C *
                                     (double)((step < P.stats_interval) ? step : P.stats_interval);
                        ...
                        if (P.progress_bar)
                            draw_progress(step, P.steps, ema_updates, ema_GBps);
                        else
                            fprintf(stderr, "[%d/%d] upd/s~%.2e  BW~%.2f GB/s\n", step, P.steps, ema_updates,
                                    ema_GBps);
                    }
                }
            }
