#!/usr/bin/env python3
"""
Benchmark sw2d_omp by sweeping rows/cols/steps/thread counts.
Produces raw and summary CSVs plus runtime/speedup/efficiency/iso-efficiency plots.
"""
...
def build_cli() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Benchmark sw2d_omp across grids and thread counts.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    ap.add_argument("--exe", default=str(Path(__file__).with_name("sw2d_omp")), help="Path to sw2d_omp executable.")
    ap.add_argument("--rows", default="200", help="Row counts (comma list or start:end:step).")
    ap.add_argument("--cols", default="200", help="Column counts.")
    ap.add_argument("--steps", default="2000", help="Time steps.")
    ap.add_argument("--threads", default="1", help="Thread counts to test.")
    ap.add_argument("--dx", type=float, default=1.0, help="Cell size x.")
    ap.add_argument("--dy", type=float, default=1.0, help="Cell size y.")
    ...
    ap.add_argument("--repeat", type=int, default=1, help="Repeat count per configuration.")
    ap.add_argument("--init", type=str, help="Optional init/prior file.")
    ap.add_argument("--out-dir", default="sw2d_omp_sweep", help="Directory for CSVs/plots.")
    ap.add_argument("--csv", default="sw2d_omp_summary.csv", help="Summary CSV filename (relative to out-dir).")
    ap.add_argument("--extra", default="", help="Additional solver flags (quoted string).")
    ap.add_argument("--keep-movies", action="store_true", help="Preserve movie stacks (default deletes them).")
...
def generate_plots(summary_entries: List[Dict], eff_targets: List[float], out_dir: Path) -> None:
    ...
    efficiency_plot = out_dir / "sw2d_omp_efficiency.png"
    isoeff_plot = out_dir / "sw2d_omp_isoefficiency.png"
    ...
    for ax, title, ylabel in [
        (ax_runtime, "sw2d_omp compute time vs threads", "Compute time (s)"),
        (ax_speed, "sw2d_omp speedup vs threads", "Speedup"),
        (ax_eff, "sw2d_omp efficiency vs threads", "Efficiency"),
    ]:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for rows_val, cols_val, steps_val, threads_val in combos:
        for rep in range(1, args.repeat + 1):
            ...
            print(f"[{run_idx}/{total_runs}] sw2d_omp ({tag})")
            start_compute = now_fn()
            result = subprocess.run(cmd, cwd=str(script_dir))
            compute_time = now_fn() - start_compute
            if result.returncode != 0:
                print(f"[error] sw2d_omp failed for {tag} (exit {result.returncode})", file=sys.stderr)
    ...
    raw_csv = out_dir / "sw2d_omp_raw.csv"
