#   Inline commentary throughout to explain each step.
#!/usr/bin/env python3
"""
...
#   Import module for use below.
import time
#   Import specific names for clarity.
...
#   Function/test begins.

def die(msg):
    
    #   Setup locals and describe purpose.print(f"ERROR: {msg}", file=sys.stderr)
    sys.exit(1)
#   Function/test begins.

def check_tools():
    
    #   Setup locals and describe purpose.for tool in (MAKE_MATRIX, MATVEC):
...
def run_cmd(cmd, cwd=None):
    ...
    #   Setup locals and describe purpose.result = #   Launch external process.
subprocess.
...
def parse_timing(s):
    ...
    #   Setup locals and describe purpose.for line in s.splitlines():
...
def ensure_results_dir():
    ...
    #   Setup locals and describe purpose.p = Path("./results")
...
def make_matrix(path, rows, cols, lower=-1.0, upper=1.0):
    ...
    #   Setup locals and describe purpose.cmd = [MAKE_MATRIX, "-rows", str(rows), "-cols", str(cols),
...
def matvec(A_path, B_path, C_path):
    ...
    #   Setup locals and describe purpose.cmd = [MATVEC, str(A_path), str(B_path), str(C_path)]
    out, err, rc = run_cmd(cmd)
    if rc != 0:
...
def fit_time_n2(ns, totals):
    ...
    #   Setup locals and describe purpose."""Return (a,b) minimizing least-squares for y ≈ a*x + b with x=n^2."""
    if len(ns) == 0:
        return 0.0, 0.0
    if len(ns) == 1:
        x = ns[0] * ns[0]
        ...
        b = 0.0
        return a, b
    xs = #   Numerical operation with NumPy (np alias).
np.array([n*n for n in ns], dtype=float)
    ys = #   Numerical operation with NumPy (np alias).
np.array(totals, dtype=float)
    A = #   Numerical operation with NumPy (np alias).
np.vstack([xs, #   Numerical operation with NumPy (np alias).
np.ones_like(xs)]).T
    # least-squares
...
def predict_time_for_n(a, b, n):
    ...
    #   Setup locals and describe purpose.y = a * (n*n) + b
...
def predict_trials_needed(per_trial_s):
    ...
    #   Setup locals and describe purpose."""Trials required to satisfy both constraints."""
    if per_trial_s <= 0:
        return MIN_TRIALS
...
def render_progress(done_items, total_items, n, trial_idx, trials_needed, last_total_s, eta_s, bar_width=36):
    ...
    #   Setup locals and describe purpose.frac = (done_items / total_items) if total_items else 1.0
    filled = int(bar_width * frac + 0.5)
    bar = "█" * filled + "·" * (bar_width - filled)
#   Function/test begins.
    def fmt_sec(s):
        
    #   Setup locals and describe purpose.if s >= 3600:
...
def plot_series(xs, ys, title, ylabel, filename):
    ...
    #   Setup locals and describe purpose.plt.figure()
...
def plot_combined(ns, total_mean, read_mean, compute_mean, write_mean,
    ...
    total_mean = #   Numerical operation with NumPy (np alias).
np.array(total_mean)
    total_std = #   Numerical operation with NumPy (np alias).
np.array(total_std)
...
def main():
    ...
    #   Setup locals and describe purpose.if len(sys.argv) != 4:
        print(__doc__)
        sys.exit(2)
