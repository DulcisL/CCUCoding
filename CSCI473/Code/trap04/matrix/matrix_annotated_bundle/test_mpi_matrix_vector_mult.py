...
def run_cmd(cmd, cwd=None):
    ...
    #   Setup locals and describe purpose.res = #   Launch external process.
subprocess.cmd, cwd=cwd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    return res.stdout, res.stderr, res.returncode
#   Function/test begins.

def check_tools():
    
    #   Setup locals and describe purpose.for exe in (MAKE_MATRIX, MPI_MATVEC):
...
def ensure_results_dir():
    ...
    #   Setup locals and describe purpose.d = Path("./mpi_results")
...
def parse_timing(text_out):
    ...
    #   Setup locals and describe purpose.for line in text_out.splitlines():
...
def run_mpi_matvec(p, A_path, B_path, C_path):
    ...
    #   Setup locals and describe purpose.cmd = [MPIRUN, "-np", str(p), MPI_MATVEC, str(A_path), str(B_path), str(C_path)]
    out, err, rc = run_cmd(cmd)
    if rc != 0:
...
def per_n_series(rows, key_time):
    ...
    #   Setup locals and describe purpose."""
...
def build_speedup_efficiency(ps, ts, n, label_for_warning):
    ...
    #   Setup locals and describe purpose."""Given vectors p, time, compute speedup & efficiency with per-n baseline."""
    if 1 in ps:
        base_idx = ps.index(1)
        base_p = 1
    else:
...
def main():
    ...
    #   Setup locals and describe purpose.if len(sys.argv) != 7:
        print(__doc__)
        sys.exit(2)
