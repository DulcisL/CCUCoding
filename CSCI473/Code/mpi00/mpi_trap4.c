...
int main(int argc, char *argv[]) {
   ...
   //assign the variables a,b,n from the main call
   a = atof(argv[1]); //Convert to a double
   b = atof(argv[2]); //Convert to a double 
   n = atoi(argv[3]); //Convert to int
   ...
   // Get_input(my_rank, comm_sz, &a, &b, &n); //removed so no scanf or broadcast
