#!/usr/bin/env python3
"""
Usage: python bench_mc_slab_omp.py --exe EXE --C C --Cc CC --H H
    ...
    --exe EXE            Path to mc_slab_omp executable (default: ./mc_slab_omp)
    --C C                Total interaction coefficient
    --Cc CC              Absorption coefficient
    --H H                Slab thickness
    --seed SEED          Base RNG seed (applied via MC_SLAB_SEED)
    --n_start N_START    Smallest particle count to benchmark
    --n_max N_MAX        Largest particle count to benchmark (doubling each step)
    --P_start P_START    Smallest thread count (OpenMP) to benchmark
    --P_step P_STEP      Step between thread counts
    --P_max P_MAX        Maximum thread count to benchmark
    --trials TRIALS      Timed trials per (N, P) pair (default: 3)
    --warmup WARMUP      Untimed warmups per (N, P) pair (default: 1)
    --eff_targets LIST   Comma-separated efficiency targets (default: 0.6,0.75,0.9)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark the OpenMP slab solver across particles and thread counts."
    )
    parser.add_argument("--exe", default="./mc_slab_omp", help="Path to mc_slab_omp executable")
    ...
    parser.add_argument("--n_start", type=int, required=True, help="Smallest particle count to benchmark")
    parser.add_argument("--n_max", type=int, required=True, help="Largest particle count (inclusive)")
    parser.add_argument("--P_start", type=int, required=True, help="Smallest thread count to benchmark")
    parser.add_argument("--P_step", type=int, required=True, help="Step between thread counts")
    parser.add_argument("--P_max", type=int, required=True, help="Maximum thread count to benchmark")
    ...
    parser.add_argument("--results-dir", dest="results_dir", default="./data", help="Directory for CSV/plot outputs")
...
def run_solver(exe: Path,
    ...
    try:
        ...
        raise RuntimeError("mc_slab_omp output was not valid JSON") from exc
...
def plot_isoeff(p_values: List[int],
    ...
    plt.title("Iso-efficiency Curves (OpenMP)")
    plt.grid(True, linestyle="--", alpha=0.4)
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_path)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    for n in n_values:
        ...
        for p in p_values:
            config_idx += 1
            print(f"[{config_idx}/{total_configs}] Benchmarking N={n}, P={p}")
    ...
    raw_path = results_dir / "bench_omp_raw_runs.csv"
    ...
    summary_path = results_dir / "bench_omp_summary.csv"
    ...
    plot_lines(p_values, runtime_series, "Runtime (s)", "Runtime vs Threads (OpenMP)", results_dir / "bench_omp_runtime.png")
    plot_lines(p_values, speedup_series, "Speedup", "Speedup vs Threads (OpenMP)", results_dir / "bench_omp_speedup.png")
    plot_lines(p_values, efficiency_series, "Efficiency", "Efficiency vs Threads (OpenMP)", results_dir / "bench_omp_efficiency.png")
    ...
    plot_isoeff(p_values, isoeff_table, results_dir / "bench_omp_isoefficiency.png")
