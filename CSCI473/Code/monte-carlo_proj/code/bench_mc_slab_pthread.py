#!/usr/bin/env python3
"""
Usage: python bench_mc_slab_pthreads.py --exe EXE --C C --Cc CC --H H
    ...
    --P_start P_START    Smallest thread count to benchmark
...
Outputs:
    - bench_raw_runs.csv        Per-trial timings and tallies
    - bench_summary.csv         Aggregate stats per (N, P)
    - bench_runtime.png         Runtime vs. thread count
    - bench_speedup.png         Speedup vs. thread count
    - bench_efficiency.png      Efficiency vs. thread count
    - bench_isoefficiency.png   Iso-efficiency curves per target
"""
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark the pthread slab solver across particles and thread counts."
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    raw_path = results_dir / "bench_raw_runs.csv"
    ...
    summary_path = results_dir / "bench_summary.csv"
    ...
    plot_lines(p_values, runtime_series, "Runtime (s)", "Runtime vs Threads", results_dir / "bench_runtime.png")
    plot_lines(p_values, speedup_series, "Speedup", "Speedup vs Threads", results_dir / "bench_speedup.png")
    plot_lines(p_values, efficiency_series, "Efficiency", "Efficiency vs Threads", results_dir / "bench_efficiency.png", yscale=None)
    ...
    plot_isoeff(p_values, isoeff_table, results_dir / "bench_isoefficiency.png")
