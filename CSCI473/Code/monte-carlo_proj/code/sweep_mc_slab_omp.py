#!/usr/bin/env python3
"""
Usage: python sweep_mc_slab_omp.py --exe EXE --C C --Cc CC --H H
    ...
    --P_start P_START    Smallest thread count (OpenMP) to sweepmark (required without --Ps)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="sweepmark the OpenMP slab solver across particles and thread counts."
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    raw_path = results_dir / "sweep_omp_raw_runs.csv"
    ...
    summary_path = results_dir / "sweep_omp_summary.csv"
    ...
    plot_lines(p_values, runtime_series, "Runtime (s)", "Runtime vs Threads (OpenMP)", results_dir / "sweep_omp_runtime.png")
    plot_lines(p_values, speedup_series, "Speedup", "Speedup vs Threads (OpenMP)", results_dir / "sweep_omp_speedup.png")
    plot_lines(p_values, efficiency_series, "Efficiency", "Efficiency vs Threads (OpenMP)", results_dir / "sweep_omp_efficiency.png")
    ...
    plot_isoeff(p_values, isoeff_table, results_dir / "sweep_omp_isoefficiency.png")
