...
/*
   ...
   Height curve:
     ...
     • Segment 4 (rest): stronger descent to desk height
...
function globalHeightForFlightTime(flightTimeMs) {
  ...
  const Hdesk = 0.0;                     // "landing" reference (desk plane)
  ...
  // Map normalized height to world space; add desk offset if needed.
  return Hdesk + Hmax * h;
}

/* --------------- Transform composition helpers (unchanged) --------------- */
...
function paperFlightMatrix(nowMs) {
  ...
  // Final snap: if we've exhausted Ttotal, pin to desk height
  if (flightTime >= Ttotal) {
    landingState.pos[1] = 0.0; // desk plane; adjust if your desk offset differs
  } else {
...
function render(now) {
  ...
  // Camera
  const eye = orbitToEye();//vec3(-3, 6.0, -6)//
